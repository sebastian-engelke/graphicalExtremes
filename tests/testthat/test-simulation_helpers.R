...
test_that("simu_px_logistic works", {
  ...
  expect_error(simu_px_logistic(n,
    idx = c(sample(1:d, n, replace = T), 1),
    d, theta = 0.2
  ))
...
test_that("simu_px_neglogistic works", {
  ...
  expect_error(simu_px_neglogistic(n,
    idx = c(sample(1:d, n, replace = T), 1),
    d, theta = 0.2
  ))
  ...
  res <- simu_px_neglogistic(7,
    idx = sample(x = 1:4, size = 7, replace = T),
    d = d, theta = 0.2
  )
...
test_that("simu_px_dirchlet works", {
  expect_error(simu_px_dirichlet(n,
    idx = c(1, 3), d,
  ...
  res <- simu_px_dirichlet(7,
    idx = sample(x = 1:4, size = 7, replace = T), d = d,
...
test_that("simu_px_tree_logistic works", {
  ...
  res <- simu_px_tree_logistic(n, theta = runif(2), A = A[[2]])
  expect_type(res, "double")
  expect_equal(dim(res), c(n, d))
})
