...
```{r, include = TRUE}
# TEMP: Stop evaluating R code at this point
# TODO: remove
knitr::opts_chunk$set(
  eval = FALSE
)
...
eglasso_loglik <- sapply(1:length(rholist), FUN = function(j) 
  loglik_HR(data=Ysim, p=.8, Gamma = eglasso_fit$Gamma[[j]], 
            graph =  eglasso_fit$graph[[j]]))

emst_fit <- emst(Ysim)

danube_loglik_tree <- loglik_HR(data=Ysim, p=.8, Gamma = emst_fit$Gamma,
...
# TEMP: Start evaluating R code at this point
# TODO: remove
knitr::opts_chunk$set(
  eval = TRUE
)
...
# Exercises
TODO: redo with new data, functions!
