...
plot(danube_emst_fit$tree, layout = danube$coords_to_plot)
```
...
```{r, fig.align='center'}
set.seed(42)
rholist = seq(1e-5, 0.04, length.out = 20)
my_eglasso <- eglasso(Gamma = Gamma_hat, rholist = rholist, 
                    eps = .5, reg_method = "glasso")

my_loglik <- sapply(1:length(rholist), FUN = function(j) 
  loglik_HR(data=X, p=.8, Gamma = my_eglasso[[j]]$Gamma, cens = FALSE, graph =  my_eglasso[[j]]$graph))

na.idx <- length(rholist)
plot(rholist[1:na.idx], my_loglik[3,], col="blue", type="b")

tree_loglik_HR <- loglik_HR(data=X, p=.8, Gamma = danube_emst_fit$Gamma, cens = FALSE, graph = danube_emst_fit$tree)
abline(h = tree_loglik_HR[3], col="blue")

my_model <- my_eglasso[[which.min(my_loglik[3,])]]

ggplot() +
  geom_point(aes(x = c(Gamma2chi(my_model$Gamma)), y = c(chi_hat))) +
  geom_abline(slope = 1, intercept = 0) +
  xlab("Fitted") +
  ylab("Empirical") +
  coord_cartesian(ylim = c(0.4, 1))
