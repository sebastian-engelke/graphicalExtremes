...
```{r, fig.align='center'}
set.seed(42)
rholist = seq(1e-5, 0.04, length.out = 20) # !!! update list
...
danube_loglik <- sapply(1:length(rholist), FUN = function(j) 
  loglik_HR(data=X, p=.8, Gamma = danube_eglasso_fit[[j]]$Gamma, 
            graph =  danube_eglasso_fit[[j]]$graph))

na.idx <- length(rholist)
plot(rholist[1:na.idx], danube_loglik[3,], col="blue", type="b")

danube_loglik_tree <- loglik_HR(data=X, p=.8, Gamma = danube_emst_fit$Gamma,
                                graph = danube_emst_fit$tree)
abline(h = danube_loglik_tree[3], col="blue")

# !!! plot 3 graphs from low to high rho

my_model <- danube_eglasso_fit[[which.min(danube_loglik[3,])]]
