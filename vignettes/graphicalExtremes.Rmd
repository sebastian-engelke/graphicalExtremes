...
::: {.example}
The $d$-dimensional Huesler--Reiss distribution [@hue1989] is parameterized by the variogram matrix $\Gamma = \{\Gamma_{ij}\}_{1\leq i,j\leq d}$. The corresponding density of the exponent measure can be written for any $k\in\{1,\dots, d\}$ as \citep[cf.,][]{Engelke2015}
\begin{equation}
...
In general, the matrices $\Gamma^{(k)}$ can be different for $k \in V$, but for example for the H\"usler--Reiss distribution, they all coincide.


::: {.example}
For the Huesler--Reiss distribution with parameter matrix $\Gamma$, the extremal variogram matrices satisfy
...
The H\"usler--Reiss Pareto distribution $\g Y$
...
Let us first suppose that a connected graph $G$ is given. For some data set, the function `fmpareto_graph_HR()` fits a Huesler--Reiss graphical model on the graph $G$.
