...
danube_flow <- igraph::graph_from_edgelist(danube$flow_connections)
plot(danube_flow, layout = danube$coords_to_plot, edge.arrow.size=.2)
```
...
Given $n$ oberservations $\g X_1, \dots, \g X_n$ of $\g X$, the function \texttt{data2mpareto} first computes the standardization in \@ref(eq:pareto) based on the empirical distribution functions $\hat F_j$ and then selects the exceedances $X_u$ according to \eqref{exc}. 
...
In this function, and in all subsequent functions, if the variable `p = NULL` (?? is this correct or other syntax) then the data is already expected to be on MPD scale, and no thresholding is performed. For the Danube date, we may therefore directly use $\g Y$ instead of $\g X$:
...
The Hammersley--Clifford theorem shown in @eng2019 yields that the density of an extremal tree model $\g Y$ on the tree $T= (V,E)$ factorizes into 
\begin{equation}
...
If provided with a tree and data, the function `fmpareto_graph_HR()` estimates the $d-1$ parametres of a H&uuml;sler--Reiss model on that tree. (??mention vario or ML estimation) (?? put example of Danube)

```{r, fig.align='center'}

danube_tree <- igraph::as.undirected(danube_flow)
my_fit <- fmpareto_graph_HR(data = X, graph = danube_tree, p = .8)

ggplot() +
  geom_point(aes(x = Gamma2chi(my_fit$Gamma), y = Gamma2chi(Gamma_hat))) +
...
In fact, taking the average of all $\hat \Gamma^{(k)}$, $k \in V$, and using $\rho_{ij} = \hat \Gamma_{ij}$ makes better use of the data and usually improves the performance of structure estimation. @eng2020 further show that $\rho_{ij} = \hat \chi_{ij}$ may also be used for consistent tree recovery, however the performance is typically inferior to extremal variogram based algorithms.


The function `emst()` estimates the extremal tree structure by a minimum spannign tree algorithm based on the different summary statistics for extremal dependence. It provides the estimated tree and the implied extremal variogram matrix through \@ref(eq:treemetric).


```{r, fig.align='center'}
my_fit <- emst(data = X, p = .8, method = "vario")
plot(my_fit$tree, layout = danube$coords_to_plot)
```
...
For H&uuml;sler--Reiss graphical models it suffices to specify the graph structure $G = (V,E)$ and all entries of the parameter matrix $\Gamma_{ij}$ for $(i,j) \in E$. The remaining entries of the matrix $\Gamma$ are then implicitely implied by the graph structure. 
...
This also works for more general decomposable graphs, where the matrix $\Gamma$ has to be specified on all cliques. The graph structure can also be implied by $\Gamma_{ij} = \text{NA}$ for $(i,j)\notin E$.
