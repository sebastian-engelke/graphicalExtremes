...
Given $n$ oberservations $\g X_1, \dots, \g X_n$ of $\g X$, the function \texttt{data2mpareto} first computes the standardization in \@ref(eq:pareto) based on the empirical distribution functions $\hat F_j$ and then selects the exceedances $\g X_u$ according to \@ref(eq:exc). 
...
The function `rmpareto_tree()` generates samples from a MPD $\g Y$ based on the exact algorithm described in @eng2019.
...
Note that we can also generate samples from the corresponding max-stable distribution with the function `rmstable_tree()`, following the exact algorithm in @dom2016.
...
# Exercises
The `flights()`dataset contains daily delays at different airports for the Southwest Airlines carriers, from January 1 until December 31, 2015.
The chosen airports are in California, Nevada, Arizona, Utah, and Texas.
The object `flights()` is a list made of:
* `data`: a `dplyr::tibble()`  with the delay data. Each row corresponds to a day and each column to an airport,
* `airports`: a `dplyr::tibble()` with the details for each airport.
* `connections`: a `dplyr::tibble()` with the details of the _undirected_ flights.
...
## Part A
1. Fit an extremal tree model, choosing threshold `p = 0.7`.
```{r, collapse=TRUE}
p <- .7
flights_emst_fit <- emst(data = data, p = p, method = "vario")
igraph::V(flights_emst_fit$graph)$name <- names(data)
...
2. Plot the tree on the US map and interpret the results.
```{r, collapse=TRUE}
ggplot() +
  geom_polygon(data = map_data("usa"),
               aes(x = long, y = lat, group = group), color = "grey65",
...
3. Compute BIC values of fitted tree model.
4. Plot the empirical $\chi$ coefficient against the $\chi$ coefficient implied by the fitted model.
5. Take the `flight_connection` (!!! change) graph and fit a HR graphical model on it.
6. Repeat steps 3., 4..
7. If you see some "outliers" in plot from Part 4 from the `flight_connection` graph, can you provide explanation?

### Part B
1. Fit an extremal graphical lasso model, choosing threshold `p = 0.7`. Choose `rholist = seq(1e-4, 0.09, length.out = 10)`.
2. Plot the tree on the US map for different values of `rho` and interpret the results.
3. Compute BIC graph for different values of `rho`.
...
5. Provide `Gamma` matrix using different methods
  * `emp_vario`
  * `emp_vario`, fix some`k = 1`
  * `chi` -> `Gamma`
  * `ML`using `ml_weight_matrix`
fit `eglasso` and plot the different BIC curves.

# !!! ask Leo about Zoom rooms
# !!! ask Leo about assistants during the tutorial


## General graphs
```{r}
Gamma <- emp_vario(flights$data %>% as.matrix(), p = p)
rholist = seq(1e-4, 0.12, length.out = 20)
flights_eglasso_fit <- eglasso(Gamma, rholist = rholist, complete_Gamma = TRUE)
est_graph <- flights_eglasso_fit$graph[[19]]
igraph::V(est_graph)$name <- names(flights$data)
...
ggplot() +
  geom_polygon(data = map_data("usa"),
               aes(x = long, y = lat, group = group), color = "grey65",
               fill = "#f9f9f9", size = 0.2) +
  geom_point(data = flights$airports,
             aes(x = LONGITUDE, y = LATITUDE, size = N_FLIGHTS),
             alpha = 1) +
  geom_curve(data = flights_connections_est,
             aes(x = LONGITUDE.origin, xend = LONGITUDE.dest,
...
flights_loglik <- sapply(1:length(rholist), FUN = function(j) 
  loglik_HR(data=flights$data, p=p, Gamma = flights_eglasso_fit$Gamma[[j]], 
            graph =  flights_eglasso_fit$graph[[j]]))


flights_loglik_tree <- loglik_HR(data=flights$data, p=p, 
...
ggplot(mapping = aes(x = rholist, y = flights_loglik[3, ])) +
  ...
  scale_x_continuous(
    ...
    labels = round(rholist, 3),
    sec.axis = sec_axis(trans=~., breaks = rholist,
                        labels = sapply(flights_eglasso_fit$graph, igraph::gsize),
                        name="Number of edges")
  )
```
```{r}
...
ggplot() +
  geom_point(aes(x = c(Gamma2chi(best_Gamma)), 
                 y = c(emp_chi(flights$data %>% as.matrix(), p = p)))) +
  geom_abline(slope = 1, intercept = 0) +
  xlab("Fitted") +
  ylab("Empirical")
```
