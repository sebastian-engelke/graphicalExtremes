...
## Trees
```{r}
flights_emst_fit <- emst(data = flights$data, p = .8, method = "vario")
...
flights_loglik <- sapply(1:length(rholist), FUN = function(j) 
  loglik_HR(data=flights$data, p=.7, Gamma = flights_eglasso_fit$Gamma[[j]], 
            graph =  flights_eglasso_fit$graph[[j]]))


flights_loglik_tree <- loglik_HR(data=flights$data, p=.7, Gamma = flights_emst_fit$Gamma,
...
ggplot() +
  geom_point(aes(x = c(Gamma2chi(best_Gamma)), y = c(emp_chi(flights$data %>%  as.matrix(), p = .7)))) +
  geom_abline(slope = 1, intercept = 0) +
  xlab("Fitted") +
  ylab("Empirical")
```
