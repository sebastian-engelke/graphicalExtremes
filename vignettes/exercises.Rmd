...
The `flights()`dataset contains daily delays (Jan 1, 2015 -- Dec 31, 2015) of Southwest Airlines at different airports in California, Nevada, Arizona, Utah, and Texas.
...
# Part A
1. Fit an extremal tree model with `emst()`, choosing threshold `p = 0.7`. 
...
```{r}
flights_loglik_tree <- loglik_HR(data=data, p=p,
...
_Hint_: The function `emp_chi()` accepts `base::matrix()` as input.
```{r, fig.align='center'}
emp_chi_mat <- emp_chi(data %>% as.matrix(), p = p)
...
```{r}
model_fit <- fmpareto_graph_HR(data = data %>% as.matrix(),
...
```{r}
flights_loglik_graph <- loglik_HR(data = data %>% as.matrix(), 
...
```{r, fig.align='center', fig.width=10}
outlier_inds <- which(Gamma2chi(model_fit$Gamma) < 0.4 & emp_chi_mat > 0.5,
                      arr.ind = TRUE)

outlier_tib <- tibble(
  ORIGIN_AIRPORT = colnames(data)[outlier_inds[, 1]],
  DESTINATION_AIRPORT = colnames(data)[outlier_inds[, 2]]
)

outlier_connections <- 
  paste0(connections$ORIGIN_AIRPORT, connections$DESTINATION_AIRPORT) %in%
  paste0(outlier_tib$ORIGIN_AIRPORT, outlier_tib$DESTINATION_AIRPORT)
...
ggplot() +
  ...
  geom_point(data = airports,
             aes(x = LONGITUDE, y = LATITUDE, size = N_FLIGHTS),
             alpha = 1) +
  geom_curve(data = connections[!outlier_connections, ],
             aes(x = LONGITUDE.origin, xend = LONGITUDE.dest,
                 y = LATITUDE.origin, yend = LATITUDE.dest,
                 size = N_FLIGHTS),
             alpha = .2, curvature = 0) +
  geom_curve(data = connections[outlier_connections, ],
             aes(x = LONGITUDE.origin, xend = LONGITUDE.dest,
                 y = LATITUDE.origin, yend = LATITUDE.dest,
                 size = N_FLIGHTS),
             alpha = .2, curvature = 0, color = "orange")
...
1. Fit an extremal graphical lasso model with `eglasso()`, choosing threshold `p = 0.7`, and `rholist = seq(1e-4, 0.09, length.out = 10)`.

```{r}
Gamma <- emp_vario(data %>% as.matrix(), p = p)
...
flights_eglasso_fit$graph <- lapply(seq_along(rholist), function(i) {
  ...
  igraph::V(est_graph)$name <- names(data)
  est_graph
})
```
...
```{r, fig.align='center'}
Gamma_vario_k_1 <- emp_vario(data = data %>% as.matrix(), k = 1, p = p)
...
```{r, fig.align='center'}
# Gamma_from_chi <- chi2Gamma(chi = emp_chi(data = data %>% as.matrix, p = p))
# !!! opposite behaviour wrt rho
Gamma_ml <- graphicalExtremes:::ml_weight_matrix(data = data %>% as.matrix,
                                                 p = p)
flights_eglasso_fit <- eglasso(Gamma_ml$est_gamma, rholist, 
                               complete_Gamma = TRUE)
