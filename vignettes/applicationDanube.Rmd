...
Throughout this document, where applicable,
we will use the Danube dataset from @asadi2015 to illustrate functions and concepts.
The dataset is provided in the package as `flights`,
...
plotDanubeIGraph(graph = danube_flow, labelStations = TRUE)
```

```{r, fig.show='hold', out.width="50%", fig.align=''}
# Plot some pairwise scatter plots
X <- as_tibble(danube$data_clustered)
ggplot(X) + geom_point(aes(x = X1, y = X2))
ggplot(X) + geom_point(aes(x = X22, y = X28))
```
...
There exist several other summary statistics for extremal dependence. The extremal variogram was introduced in @eng2020 and turns out to be particularly useful for inference of extremal graphical models discussed below. 
...
# All defaults
# TODO: directly plot igraph graphs
...
```{r}
# TODO: use function from package
...
# TODO: redo with Danube data
set.seed(42)
# (Ysim from previous example is used)
rholist <- seq(0, 0.1, length.out = 10)
eglasso_fit <- eglearn(
    data = Ysim,
    rholist = rholist,
    complete_Gamma = TRUE
...
## Exercises
For some exercises and solutions see `vignette("exercises")`.
